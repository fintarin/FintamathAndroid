  ...
  function insertBrackets(rootElem, childElem, mathText, start, end, funcName = '') {
    ...
    function createFunctionElement(funcName, elem) {
      ...
      switch (funcName) {
        ...
        case absFunction: {
          setClassName(elem, absClass);
          elem.style.borderColor = getColorWithOpacity(mathTextView.style.color, linesOpacity);
          break;
        }
        default: {
          putInBrackets(elem);
...
function toMathText(html, isEditable = false) {
  ...
  function toMathTextRec(elem) {
    switch (getClassName(elem)) {
      ...
      case absClass: {
        return absFunction + openBracket + toMathTextChildren(elem) + closeBracket;
      }
      case sqrtClass: {
...
function insertHints(elem, start = 0, end = elem.childElementCount - 1) {
  for (let i = start; i <= end; i++) {
    ...
    if (
      (className === binaryOperatorClass || className === unaryPrefixOperatorClass || className === openBracketClass) &&
...
function insertBordersRec(elem) {
  for (let i = 0; i < elem.childElementCount; i++) {
    ...
    if (getClassName(childElem) === openBracketClass && isNotEmptyTextElement(childElem.nextElementSibling)) {
